#!/usr/bin/env python3
"""
Ethereum address collector with live TXT writing.

Scans blocks 100..25,673,000 using multiple public Ethereum RPC endpoints.
For every completed batch:
1. new unique addresses are committed to SQLite;
2. the same new addresses are appended immediately to ethereum_addresses.txt;
...
DB_FILE = Path("ethereum_addresses.db")
TXT_FILE = Path("ethereum_addresses.txt")
...
class Stats:
    ...
    db_unique_addresses: int = 0
    txt_written_total: int = 0
    txt_written_session: int = 0
    last_batch_new_addresses: int = 0
...
async def load_db_state(
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        count_row = await (
            await db.execute("SELECT COUNT(*) FROM addresses")
        ).fetchone()
...
async def db_and_txt_writer(
    ...
    Each batch:
    - place candidate addresses in a temporary SQLite table;
    - select only addresses absent from the main address table;
    - commit new addresses and progress;
    - append exactly those new addresses to TXT;
    ...
    async with aiosqlite.connect(db_path) as db:
        ...
        await db.execute(
            """
            CREATE TEMP TABLE IF NOT EXISTS candidate_addresses (
                address TEXT PRIMARY KEY
            ) WITHOUT ROWID
            """
        )
        ...
        with txt_path.open(
            ...
            while True:
                ...
                for result in batch:
                    candidate_set.update(result.addresses)
                ...
                try:
                    ...
                    await db.execute(
                        "DELETE FROM candidate_addresses"
                    )

                    if candidate_set:
                        await db.executemany(
                            """
                            INSERT OR IGNORE INTO candidate_addresses(address)
                            VALUES (?)
                            """,
                            ((address,) for address in candidate_set),
                        )

                        cursor = await db.execute(
                            """
                            SELECT c.address
                            FROM candidate_addresses AS c
                            WHERE NOT EXISTS (
                                SELECT 1
                                FROM addresses AS a
                        ...
                        new_addresses = [row[0] for row in rows]
                        ...
                        if new_addresses:
                            await db.executemany(
                                ...
                                VALUES (?)
                                """,
                                ((address,) for address in new_addresses),
                            )
                ...
                if new_addresses:
                    ...
                    stats.txt_written_total += len(new_addresses)
                    stats.txt_written_session += len(new_addresses)
                    stats.last_txt_write_at = time.monotonic()

                stats.last_batch_new_addresses = len(new_addresses)
                stats.db_unique_addresses += len(new_addresses)
                ...
                if stats.committed_blocks % 1000 < len(batch):
                    ...
                    stats.db_unique_addresses = int(count_row[0])
...
def dashboard(
    ...
    summary = Table(
        title=f"Ethereum address collector — {status}",
        expand=True,
    )
    ...
    summary.add_row(
        "[green]DB unique addresses[/green]",
        f"[bold green]{stats.db_unique_addresses:,}[/bold green]",
        "[bold cyan]TXT written this run[/bold cyan]",
        f"[bold cyan]{stats.txt_written_session:,}[/bold cyan]",
    )

    summary.add_row(
        "[bold cyan]TXT total lines[/bold cyan]",
        f"[bold cyan]{stats.txt_written_total:,}[/bold cyan]",
        "Last batch new",
        f"{stats.last_batch_new_addresses:,}",
    )
    ...
    root.add_row(
        "[dim]TXT har database batchidan keyin flush va fsync qilinadi. "
...
async def async_main(args: argparse.Namespace) -> int:
    ...
    if db_count != txt_lines:
        ...
        console.print(
            ...
            f"{txt_lines:,} addresses → {args.txt}"
        )
    ...
    headers = {
        ...
        "User-Agent": "EthereumAddressCollector/5.0",
    }
    ...
    console.print(
        f"[bold green]TXT addresses:[/bold green] "
        f"{final_txt_lines:,}"
    )
    console.print(
        f"[bold green]DB unique addresses:[/bold green] "
        f"{stats.db_unique_addresses:,}"
    )

    if final_txt_lines != stats.db_unique_addresses:
        console.print(
